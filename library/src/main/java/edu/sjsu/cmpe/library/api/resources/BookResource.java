...
public class BookResource {
	...
	public final BookRepositoryInterface bookRepository;
	public static Map<Long, Book> BookMap = new HashMap<Long, Book>(); 
	...
	public Response updateBookStatus(@PathParam("isbn") LongParam isbn, @QueryParam("status") Status status) throws JMSException {
		...
		{
			System.out.println("Calling Message Broker :: " + status.getValue());
			producer(book);
		}	
		BookDto bookResponse = new BookDto(book);
	...
	public void producer(Book book) throws JMSException
	{
		String user = LibraryService.apolloUser;
		String password = LibraryService.apolloPassword;
		String host = LibraryService.apolloHost;
		int port = LibraryService.apolloPort;
		String queue = LibraryService.queueName;
		...
		Long data = book.getIsbn();
		String libName = LibraryService.libraryName;
		TextMessage msg = session.createTextMessage(libName.concat(":").concat(data.toString()));
		msg.setLongProperty("id", System.currentTimeMillis());
		producer.send(msg);
		System.out.println(msg.getText());
		BookMap.put(data, book);
		System.out.println("HashMap :: " + BookMap.size() + BookMap.get(data).getTitle());
